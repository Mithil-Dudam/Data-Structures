...
export default function Play() {
    ...
    useEffect(() => {
        const createDLL = async () => {
            ...
            if (storedSession){
                ...
                const storedElements = sessionStorage.getItem("dll_elements")
                if (storedElements){
                    setElements(JSON.parse(storedElements))
                    return 
                }
                await fetch("http://localhost:8000/dll/create", {
                    method: "POST",
                    headers: {
    ...
    const insert = async () => {
        ...
        if (response.status === 200){
            ...
            sessionStorage.setItem("dll_elements", JSON.stringify(data.elements))
        }else{
